...
contract Project {
    enum ProjectState {Onging,Successful,Failed}
    ...
    event DonationReceived(address indexed donaor,uint256 amount);
    ...
    function initialize(address _creator,string memory _description,uint256 _goalAmount,uint256 _duration) public {
        ...
        state=ProjectState.Onging;
    }

    function donate() external payable {
        require(state==ProjectState.Onging,"Project is not ongoing");
    ...
    function updateProjectState() external onlyAfterDeadline {
        require(state==ProjectState.Onging,"Project is already finalized");
